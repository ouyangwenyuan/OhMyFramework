...
[OhMyFrameworkPanelTab ("Tool Editor1", "BombMixIcon1", 5)]
public class FirstModuleEditor : ModuleEditor<MonoBehaviour> {
    ...
    public override void OnGUI () {
        ...
        using (new GUIHelper.Horizontal (GUILayout.ExpandWidth (true), GUILayout.ExpandHeight (true))) {
            using (new GUIHelper.Vertical (Styles.berryArea, GUILayout.Width (200), GUILayout.ExpandHeight (true))) {
                if (GUILayout.Button ("TagAssetBundle")) {
                    EditorApplication.ExecuteMenuItem(MenuItemNames.TAG_ASSETBUNDLE);
                };
                if (GUILayout.Button ("VIEW_ASSETBUNDLE")) {
                    EditorApplication.ExecuteMenuItem(MenuItemNames.VIEW_ASSETBUNDLE);
                };
            }
            using (new GUIHelper.Vertical (Styles.area, GUILayout.ExpandHeight (true))) {
                if (GUILayout.Button ("导出Model预制体")) {
//                    DesignerTools.ExportModels();
                };     
                if (GUILayout.Button ("生成panel模版")) {
//                    DesignerTools.CreateUIPanelBase();
                };     
                if (GUILayout.Button ("导出panel")) {
//                    DesignerTools.ExportUIwindowScript();
                };     
                if (GUILayout.Button ("导出view")) {
//                    DesignerTools.ExportMonoScript();
                };     
            }
        }
