namespace SRDebugger.Services.Implementation
{
    ...
    public class SRDebugService : IDebugService
    {
        ...
        public void LoadPinnedFromPlayerPrefs()
        {
            if (PlayerPrefs.HasKey("SRD_Pinned"))
            {
                var pinned = PlayerPrefs.GetString("SRD_Pinned").Split(';');
                foreach (var pin in pinned)
                {
                    SRDebug.Instance.PinOption(pin);
                }
            }
        }
        
        public void UnpinAllFromPlayerPrefs()
        {
            if (PlayerPrefs.HasKey("SRD_Pinned"))
            {
                var pinned = PlayerPrefs.GetString("SRD_Pinned").Split(';');
                foreach (var pin in pinned)
                {
                    SRDebug.Instance.UnpinOption(pin);
                }
            }
        }
    }
